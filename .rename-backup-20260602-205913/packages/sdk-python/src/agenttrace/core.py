...
class _TraceContext:
    ...
    def _record_trace(self, latency_ms: int) -> None:
        ...
        context = self.options.get("context") if isinstance(self.options.get("context"), TraceContext) else None
        provided_parent = self.options.get("parent_id")
        if context is not None:
            trace_id = context.trace_id
            parent_id = context.parent_span_id
            base_meta = self.options.get("metadata") or {}
            ctx_meta = context.metadata or {}
            self.options["metadata"] = {**ctx_meta, **base_meta}
        else:
        ...
        if context is not None:
            self.agent.storage.create_trace_context(trace_id, parent_id, context.metadata)
