...
function scanAgents(json, verbose) {
  ...
  try {
    ...
    for (const line of ps.split('\n')) {
      ...
      if (agentType) {
        ...
        agents.push({ pid: parts[1] || '0', name: extractName(line, isNode ? 'node' : 'python'), cmdline: line.trim(), runtime: isNode ? 'node' : 'python', platform: 'wsl', framework, agentType });
      }
    }
  } catch { /* ignore */ }
  ...
  try {
    const tasklist = execSync('cmd.exe /c tasklist /fo csv /nh 2>nul', { encoding: 'utf8', timeout: 5000 });
...
function extractName(cmdline, runtime) {
