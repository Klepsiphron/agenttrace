...
import { AgentTrace, getSchemaVersion } from '@agenttrace/sdk';
...
describe('CLI e2e (real SQLite, all commands + flags)', () => {
  ...
  let dashStartSpy: ReturnType<typeof vi.spyOn> | undefined;
  let dashboardMod: typeof import('@agenttrace-io/dashboard') | undefined;
  ...
  beforeEach(async () => {
    ...
    // dynamic to avoid static top level load of dashboard dist in some vitest resolutions
    dashboardMod = await import('@agenttrace/dashboard');
  ...
  async function invokeMain(): Promise<void> {
    ...
    if (res && typeof (res as Promise<unknown>).then === 'function') {
      await res.catch(() => {
        /* cli error path already logged to errs */
      });
    }
  }
  ...
  it('export json (default), csv, otel, --output file, --run-id', async () => {
    ...
    resetCapture();
    process.argv = ['node', 'agenttrace', 'export', '--format', 'otel'];
    await invokeMain();
    out = logs.join('\n');
  ...
  it('tree --trace-id (with parent/child), json, missing id errors', async () => {
    ...
    expect(JSON.stringify(tree)).toContain('child-trace');
  });
  ...
  it('runs/traces/stats/costs/alerts/tree/evaluate/migrate all support --json empty cases', async () => {
    ...
    process.argv = ['node', 'agenttrace', 'evaluate'];
    await invokeMain();
    const eout2 = logs.join('\n');
    expect(eout2).toContain('No traces to evaluate.');
